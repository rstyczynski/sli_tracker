...
| `.github/actions/sli-event/emit_curl.sh` | Session token support (`x-security-token`) | Complete |
...
- **Session token support**: `emit_curl.sh` reads `security_token_file` from the profile; when present, adds `x-security-token` header. Backward-compatible with API-key-only profiles.
...
Implementation follows the approved design. `oci-auth-mode: none` bypasses the token_based exit-on-missing-oci in `oci_profile_setup.sh` (line 58–63 guard). Session token header is only added when `security_token_file` exists.
