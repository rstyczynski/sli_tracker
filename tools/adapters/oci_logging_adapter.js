...
// Example OCI Logging target adapter for handler-based router processing.
...
function resolveDestinationTarget(destination, destinationMap) {
    if (!isObject(destinationMap)) {
        throw new Error('OCI Logging adapter destinationMap must be an object');
    }
    ...
    throw new Error(`OCI Logging adapter has no target configured for destination "${exactKey}"`);
}

function createOciLoggingAdapter(options = {}) {
    if (!isObject(options)) {
        throw new Error('OCI Logging adapter options must be an object');
    }
    const destinationMap = options.destinationMap || {};
    const emit = typeof options.emit === 'function' ? options.emit : async () => {};
    const supportedTypes = new Set(['oci_logging', 'oci_log']);
